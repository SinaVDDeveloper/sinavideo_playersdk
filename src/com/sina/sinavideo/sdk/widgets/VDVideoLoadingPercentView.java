...
public class VDVideoLoadingPercentView extends TextView implements VDBaseWidget, OnLoadingListener, OnBufferingUpdateListener {
    ...
    public VDVideoLoadingPercentView(Context context, AttributeSet attrs) {
        super(context, attrs);
        // TODO Auto-generated constructor stub
        TypedArray typedArr = context.obtainStyledAttributes(attrs, R.styleable.VDVideoLoadingPercentView);
        if (typedArr != null) {
            for (int i = 0; i < typedArr.getIndexCount(); i++) {
                if (typedArr.getIndex(i) == R.styleable.VDVideoLoadingPercentView_loadingText) {
                    mLoadingText = typedArr.getString(R.styleable.VDVideoLoadingPercentView_loadingText);
                } else if (typedArr.getIndex(i) == R.styleable.VDVideoLoadingPercentView_preLoadingText) {
                    mPreLoadingText = typedArr.getString(R.styleable.VDVideoLoadingPercentView_preLoadingText);
                }
            }
            typedArr.recycle();
        }
