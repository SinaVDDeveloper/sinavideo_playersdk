 ...
 * @author sunxiao
 */
public class VDVideoLockScreenView extends ImageView implements View.OnClickListener, OnFullScreenListener,
    ...
    public VDVideoLockScreenView(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArr = context.obtainStyledAttributes(attrs, R.styleable.VDVideoLockScreenView);

        if (typedArr != null) {
            for (int i = 0; i < typedArr.getIndexCount(); i++) {
                int resID = -1;
                if (typedArr.getIndex(i) == R.styleable.VDVideoLockScreenView_LockOpenImg) {
                    resID = typedArr.getResourceId(R.styleable.VDVideoLockScreenView_LockOpenImg, -1);
                    if (resID != -1) {
                        mBackGroundOpenID = resID;
                    }
                } else if (typedArr.getIndex(i) == R.styleable.VDVideoLockScreenView_LockCloseImg) {
                    resID = typedArr.getResourceId(R.styleable.VDVideoLockScreenView_LockCloseImg, -1);
                    if (resID != -1) {
    ...
    public VDVideoLockScreenView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
    	...
    	if(controller!=null)controller.addOnScreenOrientationChangeListener(this);
    ...
    private void registerListener() {
        ...
        if(controller!=null)controller.addOnFullScreenListener(this);
    }
    ...
    private void showLockOreintationView() {
    	VDVideoViewController controller = VDVideoViewController.getInstance(this.getContext());
        if (controller!=null &&controller.getLayerContextData().getLayerType() == VDVideoViewLayerContextData.LAYER_COMPLEX_NOVERTICAL)
        {
            // 只横不竖的时候，不显示当前视图
    ...
    public void onFullScreen(boolean isFullScreen, boolean isFromHand) {
        ...
        VDLog.i("screen_oreintation", "onFullScreen isFullScreen " + isFullScreen + " , isFromHand = " + isFromHand
        ...
        if (vg != null) {
            try {
                ...
                if (mContainer != null) {
                    ...
                    ViewGroup.LayoutParams params = new FrameLayout.LayoutParams(LayoutParams.WRAP_CONTENT, -2,
                            Gravity.CENTER);
    ...
    public void onScreenOrientationVertical() {
        VDLog.i("VDVideoLockScreenView", "onScreenOrientationVertical--> " + mOrientation);
        boolean portrait = VDVideoFullModeController.getInstance().getIsPortrait();
        boolean lock = VDVideoFullModeController.getInstance().getIsScreenLock();
    ...
    public void onScreenOrientationHorizontal() {
        VDLog.i("VDVideoLockScreenView", "onScreenOrientationHorizontal--> " + mOrientation);
        boolean portrait = VDVideoFullModeController.getInstance().getIsPortrait();
        boolean lock = VDVideoFullModeController.getInstance().getIsScreenLock();
